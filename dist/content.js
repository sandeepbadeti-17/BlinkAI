(() => {
  ...
  function ensureOutputPanel() {
    ...
    modelTag.textContent = MODELS[activeModelIdx].label;
  ...
  function doRequest(customPrompt, prefix) {
    ...
    const payload = typed ? typed + ":\n\n" + currentSelection : (prefix || "") + currentSelection;
    const displayText = typed || currentSelection.substring(0, 100) + (currentSelection.length > 100 ? "\u2026" : "");
    isProcessing = true;
    removeToolbar();
    ensureOutputPanel();
    ...
    chrome.runtime.sendMessage(
      ...
      (response) => {
        ...
        const text = isErr ? "\u274C " + (chrome.runtime.lastError?.message || response?.error || "No response.") : response.summary || "\u26A0\uFE0F Empty response";
  ...
  function doFollowUp(chatInput, sendBtn) {
    ...
    const payload = conversationHistory.map((m) => (m.role === "user" ? "User" : "Assistant") + ": " + m.content).join("\n\n");
    chrome.runtime.sendMessage(
      { type: "BLINK_SUMMARY", payload, model: MODELS[activeModelIdx].id },
      (response) => {
        isProcessing = false;
        ...
        hideThinking();
        const isErr = !!(chrome.runtime.lastError || !response || response.error);
        const aiText = isErr ? "\u274C " + (chrome.runtime.lastError?.message || response?.error || "No response.") : response.summary || "\u26A0\uFE0F Empty response";
  ...
  function makeDraggable(el, handle) {
    ...
    handle.addEventListener("mousedown", (e) => {
      ...
      e.preventDefault();
      const move = (e2) => {
        el.style.left = Math.max(0, Math.min(window.innerWidth - el.offsetWidth, ol + e2.clientX - sx)) + "px";
        el.style.top = Math.max(0, Math.min(window.innerHeight - el.offsetHeight, ot + e2.clientY - sy)) + "px";
      };
      const up = () => {
  ...
  document.addEventListener("mouseup", (e) => {
    if (toolbar && toolbar.contains(e.target) || outputPanel && outputPanel.contains(e.target)) return;
